...
notitionOrNot = function () {
    ...
    if (document.querySelector("#noteAll").checked == false) {
        document.querySelector("#notationButton").disabled = false;
    };
};
notitionOrNot();
...
numero = Number();
remove = function (nowHere) {
    //nowHere = a küldés 'this'-e
    nowHere_ = nowHere
    numero = nowHere.getAttribute("name");
    temporaryContainer.splice(numero, 1);
    area = document.querySelector("#notations");
    target = document.querySelector(`#notations span[name='${numero}']`)
    area.removeChild(target);

    myLength = document.querySelectorAll("#notations span").length
    //area = document.querySelectorAll("#notations tr")[myLength - 1];
    //sector = area.querySelector("td:last-child");
    for (let i = 0; i < myLength; i++) {
        sector = document.querySelectorAll("#notations span")[i];
...
notation = function (wordOfMonkey) {
    ...
    newSpan.innerHTML = `${wordOfMonkey}`;
    myLength = document.querySelectorAll("#notations span").length;
    newSpan.setAttribute("onclick", "remove(this)");
    newSpan.setAttribute("name", myLength - 1);
...
doEmptyTheNotation = function () {
    area = document.querySelector("#divForNotations");
    sector = document.querySelector("#notations");
    area.removeChild(sector);
    newDiv = document.createElement("div");
    newDiv.id = "notations";
    area.appendChild(newDiv);
